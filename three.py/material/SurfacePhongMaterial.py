...
#TODO: refactor to get rid of copying of redundant shader code
class SurfacePhongMaterial(Material):
    ...
    def __init__(self, color=[1,1,1], alpha=1, texture=None, wireframe=False, lineWidth=1, useVertexColors=False, alphaTest=0):
        ...
        {
            //values for specular lighting
            float specularStrength = 0.5;
            vec3 camPos = cameraPosition;//Figure out which part of the matrix
            ...
            else if ( light.isDirectional )
            {
                ...
                return light.color * (light.strength + specular) * cosAngle;
            }
            else if ( light.isPoint )
            {
                ...
                float spec = pow(max(dot(viewDir, reflectDir),0.0),32);
                vec3 specular = specularStrength * spec * light.color;
                return light.color * (light.strength + specular) * cosAngle * attenuation;
            }
